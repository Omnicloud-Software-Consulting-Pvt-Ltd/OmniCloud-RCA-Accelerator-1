...
const ENVIRONMENTS: { id: Environment; label: string; description: string; icon: string }[] = [
  ...
  },
  {
    id: "production",
    label: "Production",
    description: "Live enterprise environment",
    icon: "◈",
  },
];
    ...
    <div className={cn("w-full", className)}>
      ...
      >
        ...
        <motion.div
          ...
          style={{
            width: "calc(50% - 4px)",
            background: value === "sandbox"
              ? "linear-gradient(135deg, rgba(30,144,255,0.2) 0%, rgba(0,212,255,0.12) 100%)"
              : "linear-gradient(135deg, rgba(0,200,117,0.2) 0%, rgba(0,112,214,0.12) 100%)",
            border: value === "sandbox"
              ? "1px solid rgba(30,144,255,0.35)"
              : "1px solid rgba(0,200,117,0.35)",
            boxShadow: value === "sandbox"
              ? "0 0 16px rgba(30,144,255,0.2), inset 0 1px 0 rgba(255,255,255,0.08)"
              : "0 0 16px rgba(0,200,117,0.2), inset 0 1px 0 rgba(255,255,255,0.08)",
          }}
          animate={{
            x: value === "sandbox" ? 4 : "calc(100% + 0px)",
          }}
                  ...
                  <AnimatePresence>
                    {isActive && (
                      <motion.span
                        ...
                        style={{ background: "#00C875", boxShadow: "0 0 6px rgba(0,200,117,0.8)" }}
                      />
                    )}
                  </AnimatePresence>
                )}
                  ...
                  <AnimatePresence>
                    {isActive && (
                      <motion.span
                        ...
                        style={{ boxShadow: "0 0 6px rgba(30,144,255,0.8)" }}
                      />
                    )}
                  </AnimatePresence>
                )}
          ...
          {value === "production" ? (
            <>
              ...
              <span className="text-[11px]" style={{ color: "var(--text-muted)" }}>
                Connecting to <span className="font-medium" style={{ color: "#00C875" }}>Production</span> Salesforce org
              </span>
            </>
          ) : (
            <>
              ...
              <span className="text-[11px]" style={{ color: "var(--text-muted)" }}>
                Connecting to <span className="text-brand-400 font-medium">Sandbox</span> Salesforce org
              </span>
            </>
          )}
        </motion.div>
