...
/* ── Callback content (must be in Suspense due to useSearchParams) ── */
function CallbackContent() {
  ...
  useEffect(() => {
    ...
    if (sfError) {
      fail(sfErrorDesc ?? sfError);
      return;
    }

    if (!code || !state) {
      fail("No authorization code received from Salesforce.");
      return;
    }

    // Send full state to server — server verifies HMAC and derives environment
